...
from typing import Any, Dict, Optional, Set # Добавлен Optional
...
async def main() -> None:
    ...
    voices = [
        TtsVoice(
            ...
            installed=True,
            version=None,
            # ИЗМЕНЕНИЕ 1: Использование get_bcp47_lang
            languages=[
                get_bcp47_lang(
                    voice_info.get("language", {}).get(
                        "code",
                        voice_info.get("espeak", {}).get(
                            "voice", voice_name.split("_")[0]
                        ),
                    )
                )
            ],
    ...
    for custom_voice_name in custom_voice_names:
        ...
        with open(custom_config_path, "r", encoding="utf-8") as custom_config_file:
            ...
            # ИЗМЕНЕНИЕ 2: Использование get_bcp47_lang для кастомных голосов
...
# НОВАЯ ФУНКЦИЯ: Добавлена ваша функция для нормализации
def get_bcp47_lang(lang_code: Optional[str]) -> str:
    """Converts piper language code (e.g., en_US) to BCP 47 (en-US)."""
    if not lang_code:
        return ""
